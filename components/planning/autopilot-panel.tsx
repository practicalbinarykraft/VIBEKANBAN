...
const statusConfig: Record<AutopilotStatus, { icon: React.ReactNode; label: string }> = {
  ...
  RUNNING: { icon: <Loader2 className="h-5 w-5 animate-spin text-blue-500" />, label: "Processing batch..." },
  WAITING_APPROVAL: { icon: <PauseCircle className="h-5 w-5 text-yellow-500" />, label: "Waiting for approval" },
  DONE: { icon: <CheckCircle className="h-5 w-5 text-green-500" />, label: "All batches complete" },
...
export function AutopilotPanel({
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-6" data-testid="autopilot-panel">
      ...
      {/* Current Batch Info */}
      {currentBatch && (
        <div className="mb-4 rounded-md border border-border/50 p-4" data-testid="autopilot-current-batch">
          ...
          <div className="space-y-1">
            <p className="text-xs font-medium text-muted-foreground">Tasks ({currentBatch.tasks.length}):</p>
            <ul className="max-h-32 list-inside list-disc overflow-y-auto text-sm">
              {currentBatch.tasks.map((task, idx) => (
                <li key={idx} className="truncate text-muted-foreground">{task}</li>
              ))}
            </ul>
          </div>
        </div>
      ...
      <div className="flex gap-2">
        {status === "IDLE" && (
          <Button onClick={onStart} disabled={isStarting} className="flex-1" data-testid="autopilot-start-button">
            {isStarting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Starting...</> : "Start Autopilot"}
          </Button>
        )}
        ...
        {(status === "RUNNING" || status === "WAITING_APPROVAL") && (
          <Button onClick={onCancel} disabled={isCanceling} variant="outline" data-testid="autopilot-cancel-button">
            {isCanceling ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Canceling...</> : "Cancel"}
          </Button>
        )}
        ...
        {status === "FAILED" && (
          <Button onClick={onStart} disabled={isStarting} variant="outline" className="flex-1" data-testid="autopilot-retry-button">
            {isStarting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Retrying...</> : "Retry"}
          </Button>
        )}
      </div>
    </div>
